...
5. `manifest.json`'daki `displayName`/`description`'ı TR ve EN olarak doldurun — tek dilli manifest kabul edilmez. Skorlama mantığınızın ürettiği her `Metrics` anahtarı için `metricLabels`'a da (hem `manifest.json`'da hem C# `Manifest`'te) TR+EN etiket ekleyin — aksi halde sonuç/ilerleme/PDF rapor ekranlarında o metrik İngilizce ve mekanik görünür (bkz. `module-development` skill § 6).
