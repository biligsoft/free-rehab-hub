...
- **Kamera erişimi bu geliştirme makinesinde hâlâ çalışmıyor — kök neden daha net ama tam çözülmedi (Faz 8 sonrası, F8.22'de derinleştirildi).** Faz 8'de kullanıcı `sudo usermod -aG video` yaptı ve yeniden oturum açtı; kontrol edilince `/dev/video0`'da zaten `user:emre:rw-` ACL'i olduğu görüldü (muhtemelen systemd-logind "uaccess", video grubundan bağımsız olarak baştan beri vardı) — yani izin hiç asıl engel değilmiş. F8.22'de `wpctl`/`pw-cli` ile daha derin incelendi: WirePlumber aynı fiziksel USB kamerayı (Azurewave `ov9734`, `13d3:56f9`) hem `monitor.v4l2` hem `monitor.libcamera` ile AYNI ANDA yönetiyordu (ikisi de `hardware.video-capture` profilinin "wants" listesinde) — bu gereksiz çift-yönetim `~/.config/wireplumber/wireplumber.conf.d/51-disable-libcamera-monitor.conf` override'ıyla (kullanıcının onayıyla, `wireplumber`/`pipewire`/`pipewire-pulse` servisleri yeniden başlatılarak) düzeltildi ve kalıcı olarak bırakıldı — gerçek, bağımsız bir mimari düzeltme, ama **asıl "meşgul" hatasını çözmedi**. Tam temiz bir durumda (fuser hiçbir işlem göstermiyor) bile hem `ffmpeg -f v4l2` hem `gst-launch-1.0 pipewiresrc` (PipeWire'ın KENDİ yolu) aynı "Device or resource busy" hatasını veriyor — demek ki mesele iki farklı sürecin çakışması değil, PipeWire'ın kendi v4l2 monitörünün cihazı SADECE VAR OLDUĞU için (kaynağı graph'a açığa çıkarmak için) sürekli açık tutması ve bu spesifik UVC sürücüsünün/cihazın ikinci HİÇBİR açma denemesine (kendi `pipewiresrc`'i dahil) izin vermemesi. Kesin çözüm yolu bulundu ama denenmedi: `monitor.v4l2`'yi de aynı şekilde devre dışı bırakmak — kullanıcıyla konuşulup bilinçli olarak durduruldu, çünkü bu günlük kullanılan masaüstü makinede tarayıcı/video görüşme gibi PipeWire-tabanlı diğer kamera kullanımlarını da kalıcı olarak bozar; kapsam sadece bu projenin ihtiyacına göre orantısız büyük bir tradeoff. Detay için bkz. `docs/PROGRESS.md` F8.22. Faz 5'in kamera-tabanlı modülü bu makinede hâlâ uçtan uca (gerçek kamera görüntüsüyle) test edilemiyor; sentetik/statik görüntü ile pipeline testi yapılabilir (F5.12'de yapıldığı gibi).
