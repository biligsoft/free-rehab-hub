...
- **TTS `--headless` modda hiçbir platformda görünmüyor (Faz 8 sonrası, F8.21 spike'ı ile doğrulandı, F8.23'te Windows/macOS'a genişletildi):** `DisplayServer.HasFeature(TextToSpeech)` `--headless` modda platform fark etmeksizin her zaman `false` dönüyor — TTS'i test etmek PENCERELİ bir çalıştırma gerektiriyor (Linux'ta Xvfb altında ama `--headless` olmadan, Windows/macOS'ta zaten normal, bu runner'ların gerçek bir masaüstü oturumu var). Linux'ta ayrıca **bir motor bulgusu**: `DisplayServer.TtsGetVoicesForLanguage(dil)` istenen dilde HİÇ ses yoksa dahili bir Godot hatası logluyor (`ERROR: Parameter "synth" is null", tts_linux.cpp`) — non-fatal (boş dizi dönüyor), ve `TtsAutoload.Speak()`'ün mevcut boş-ses-kimliği fallback'i bu durumda da hatasız çalışmaya devam ediyor (varsayılan sese düşüyor). **Doğrulandı:** Linux (Fedora dev makinesi + CI `tts-check` job'u), ve **F8.23'te Windows/macOS'ta da CI'da gerçek Godot ile doğrulandı** — `tts-check` job'u üç platformda da (ilk denemede) başarılı: `DisplayServer.HasFeature(TextToSpeech) = true`, `TtsAutoload.Speak()`/`Stop()` çökmeden çalıştı. Godot mono zip asset adları (`Godot_v4.7-stable_mono_win64.zip`, `Godot_v4.7-stable_mono_macos.universal.zip` → `Godot_mono.app/Contents/MacOS/Godot`) GitHub'ın releases API'sinden gerçek dosya listesi çekilerek doğrulandı, tahmin edilmedi. **Hâlâ bilinmeyen:** GitHub Actions runner'larının Windows/macOS TTS motorlarında (SAPI5/NSSpeechSynthesizer) fiilen Türkçe ses paketi olup olmadığı — job loglarının tam metni admin yetkisi gerektirdiğinden bu ayrıntı görülemedi, ama zaten önemli olan (uygulamanın çökmemesi) doğrulandı; hedef klinik bilgisayarda Türkçe ses paketi olmasa bile aynı graceful-degradation davranışı geçerli.
...
- **Kamera erişimi bu geliştirme makinesinde hâlâ çalışmıyor — gerçek kök neden F8.32'de nihayet bulundu, FreeRehabHub'la hiç ilgisi yok.** F8.22'de "PipeWire'ın kendi `monitor.v4l2`'si cihazı sadece var olduğu için açık tutuyor" teşhisi konulmuştu (bkz. aşağıdaki tarihsel not) — F8.32'de bu teşhis test edilmek istendiğinde (`monitor.v4l2`'yi geçici olarak devre dışı bırakıp `wpctl status`'ta PipeWire'ın kamerayı artık hiç yönetmediği doğrulandıktan SONRA) `ffmpeg -f v4l2` **yine de** "Device or resource busy" verdi — yani F8.22'nin teşhisi YANLIŞMIŞ (ya da en azından eksikmiş). Gerçek sebep: `motion_bridge-motion-bridge` adında, kullanıcının kendi kurduğu, bu projeyle **hiçbir ilgisi olmayan** bir Docker container (`docker compose` projesi `motion_bridge`, muhtemelen bir hareket algılama/bildirim köprüsü) `/dev/video0`'ı doğrudan cihaz geçişiyle (`--device=/dev/video0`) mount edip **kernel seviyesinde tekelen** tutuyor — `RestartPolicy: unless-stopped` olduğu için sürekli (F8.32'de tespit edildiğinde 4 gündür kesintisiz) çalışıyor. Bu, root yetkisiyle çalıştığı için normal kullanıcı olarak `fuser` ile hiç görünmüyordu (F8.22'nin "fuser hiçbir işlem göstermiyor" bulgusu bu yüzden yanıltıcıydı — fuser, başka kullanıcıya ait process'lerin dosya tanıtıcılarını göremiyor). Kullanıcıya soruldu ("container'ı durduralım mı, kamera testi için?") → **"Şimdilik dokunma (Recommended)"** — `motion_bridge` kendi amacı için önemli, FreeRehabHub'ın kamera testi ihtiyacı bunu durdurmaya değmez. **Sonuç:** PipeWire `monitor.v4l2`/`monitor.libcamera` çift-yönetimi (F8.22) gerçek ama alakasız bir bulguydu — asıl engel her zaman bu Docker container'ıymış, hedef donanımı (klinik bilgisayarlarda böyle bir container olmayacak) hiç etkilemiyor. Kullanıcı `motion_bridge`'i durdurmaya karar verirse (`docker stop`, `unless-stopped` politikası nedeniyle reboot dahil bir daha otomatik başlamaz) kamera muhtemelen gerçekten serbest kalır — F8.22'nin PipeWire düzeltmesine artık gerek yok. Detay için bkz. `docs/PROGRESS.md` F8.32 (ve tarihsel bağlam için F8.22). Faz 5'in kamera-tabanlı modülü bu makinede hâlâ uçtan uca (gerçek kamera görüntüsüyle) test edilemiyor; sentetik/statik görüntü ile pipeline testi yapılabilir (F5.12'de yapıldığı gibi).
...
- ~~TTS'in Türkçe ses kalitesi/varlığı Windows/macOS'ta doğrulanmadı~~ — **F8.23'te CI ile kapatıldı.** `TtsAutoload`, Godot'un yerleşik `DisplayServer` TTS'ine sarılı (sıfır yeni bağımlılık); bu Linux geliştirme makinesinde speech-dispatcher/espeak-ng üzerinden gerçek Türkçe konuşma ürettiği zaten doğrulanmıştı. F8.23'te `tts-check` CI job'u Windows/macOS'a da genişletildi ve **her iki platformda da ilk denemede başarılı oldu**: `DisplayServer.HasFeature(TextToSpeech) = true`, `TtsAutoload.Speak()`/`Stop()` çökmeden çalıştı (bkz. §13). Yani Windows'ta SAPI5'e, macOS'ta NSSpeechSynthesizer'a sarılan kod yolu artık üç platformda da CI ile doğrulanmış durumda — mediapipe/SQLCipher ile aynı doğrulama seviyesine ulaştı (bkz. §13, F8.14/F8.16). **Hâlâ tam kapanmayan tek ayrıntı:** CI runner'larında fiilen Türkçe ses paketi olup olmadığı (job log'ları admin yetkisi gerektirdiğinden görülemedi) — ama bu önemli değil, çünkü kod zaten Türkçe ses YOKKEN bile çökmeyecek şekilde tasarlandı ve bu davranış doğrulandı; hedef klinik bilgisayarda gerçek Türkçe ses kalitesi (doğal telaffuz vb.) hâlâ gerçek donanımda kulakla değerlendirilmeli, ama bu bir "çöküyor mu" riski değil, bir kullanıcı deneyimi kalite sorusu.
