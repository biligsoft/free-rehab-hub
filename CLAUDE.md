...
- **TTS `--headless` modda hiçbir platformda görünmüyor + dil eşleşmezse dahili motor hatası (Faz 8 sonrası, F8.21 spike'ı ile doğrulandı):** `DisplayServer.HasFeature(TextToSpeech)` `--headless` modda platform fark etmeksizin her zaman `false` dönüyor — TTS'i test etmek PENCERELİ bir çalıştırma gerektiriyor (Linux'ta Xvfb altında ama `--headless` olmadan, Windows/macOS'ta zaten normal). Ayrıca **yeni bir gerçek motor bulgusu**: `DisplayServer.TtsGetVoicesForLanguage(dil)` istenen dilde HİÇ ses yoksa dahili bir Godot hatası logluyor (`ERROR: Parameter "synth" is null", tts_linux.cpp`) — non-fatal (boş dizi dönüyor), ve `TtsAutoload.Speak()`'ün mevcut boş-ses-kimliği fallback'i bu durumda da hatasız çalışmaya devam ediyor (varsayılan sese düşüyor). **Doğrulandı:** Linux (Fedora dev makinesi + `.github/workflows/ci.yml`'in yeni `tts-check` job'u, ubuntu-latest, gerçek Godot ile). **Henüz doğrulanmadı:** Windows/macOS'ta aynı davranış — kullanıcıyla konuşulup kapsam bilinçli olarak sadece ubuntu-latest'le sınırlandı (Godot mono binary'sinin Windows/macOS asset adları ve macOS'ta app-bundle yolu yerel olarak doğrulanamadığından), ayrı bir adımda genişletilebilir.
...
- **TTS'in Türkçe ses kalitesi/varlığı Windows/macOS'ta doğrulanmadı** (Faz 7, F7.08; kısmen F8.21'de ilerletildi) — `TtsAutoload`, Godot'un yerleşik `DisplayServer` TTS'ine sarılı (sıfır yeni bağımlılık); bu Linux geliştirme makinesinde speech-dispatcher/espeak-ng üzerinden gerçek Türkçe konuşma ürettiği doğrulandı, ayrıca F8.21'de CI'da (ubuntu-latest, `tts-check` job'u) gerçek Godot ile pencereli çalıştırılıp Türkçe ses paketi hiç yokken bile `TtsAutoload.Speak()`'ün çökmediği doğrulandı (bkz. §13). Ama Windows'ta SAPI5'e, macOS'ta NSSpeechSynthesizer/AVSpeechSynthesizer'a sarılıyor — hedef klinik bilgisayarda Türkçe ses paketi kurulu olmayabilir (özellikle Windows dil paketi gerektirebilir), ve bu iki platform CI'da **henüz denenmedi** (kullanıcıyla konuşulup kapsam bilinçli olarak ubuntu-latest'le sınırlandı, Godot mono binary'sinin Windows/macOS asset adları yerel olarak doğrulanamadığından) — SQLCipher/mediapipe'ın aksine (bkz. §13, F8.14/F8.16) bu risk kısmen açık.
